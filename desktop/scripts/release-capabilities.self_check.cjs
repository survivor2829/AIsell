...
const validWorkflowVerification = new Set(["pending", "verified"]);
...
assert.deepEqual(Object.keys(moments.workflows || {}).sort(), ["dailyAutomation", "perPostInteraction"]);
...
assert.equal(moments.workflows.perPostInteraction.localLiveVerification, "verified");
