...
def trim_material_timeline(
    timeline: dict[str, Any],
    required_duration_ms: int,
    *,
    required_evidence_refs: Iterable[str] | None = None,
) -> dict[str, Any]:
    required_duration_ms = max(1, int(required_duration_ms))
    _contract(
        int(timeline.get("selected_duration_ms") or 0) >= required_duration_ms,
    ...
    evidence_keys = []
    for value in required_evidence_refs or []:
        key = _clean_text(value, 128).split(":", 1)[0]
        if key and key not in evidence_keys:
            evidence_keys.append(key)
    if evidence_keys:
        _contract(
            required_duration_ms >= len(evidence_keys),
            "auto_mix_material_too_short",
            "合格素材不足以同时保留全部口播事实镜头。",
        )
        mandatory_indexes = []
        for key in evidence_keys:
            index = next(
                (
                    ...
                    if key
                    in {
                        _clean_text(item.get("evidence_ref"), 128),
                        _clean_text(item.get("segment_id"), 128),
                    }
                ),
                None,
            )
            ...
            if index not in mandatory_indexes:
                mandatory_indexes.append(index)
        allocations = {index: 0 for index in range(len(segments))}
        remaining = required_duration_ms
        for ordinal, index in enumerate(mandatory_indexes):
            remaining_mandatory = len(mandatory_indexes) - ordinal
            share = max(1, remaining // max(1, remaining_mandatory))
            capacity = int(segments[index]["target_duration_ms"])
            length = min(capacity, share)
            allocations[index] = length
            remaining -= length
        fill_order = [
            *mandatory_indexes,
            *[index for index in range(len(segments)) if index not in mandatory_indexes],
        ]
        for index in fill_order:
            if remaining <= 0:
                break
            capacity = int(segments[index]["target_duration_ms"])
            available = max(0, capacity - allocations[index])
        ...
        ordered_indexes = [
            ...
            *[
                ...
                if index not in mandatory_indexes and allocations[index] > 0
            ],
        ]
        selected = [
            (segments[index], allocations[index])
        ...
        for item in segments:
            if remaining <= 0:
                break
            length = min(int(item["target_duration_ms"]), remaining)
            selected.append((item, length))
            remaining -= length
    output = []
    cursor = 0
    for raw, length in selected:
        item = dict(raw)
        item["source_end_ms"] = int(item["source_start_ms"]) + length
        item["timeline_start_ms"] = cursor
        item["timeline_end_ms"] = cursor + length
