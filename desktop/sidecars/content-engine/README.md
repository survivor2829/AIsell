...
- `ffprobe` 是动态能力：worker 只在运行环境发现可信的绝对路径可执行文件时公布 `asset_media_probe=true`。本阶段不把 ffprobe 打进固定 runtime，因此干净机器上可能显示“媒体探测组件未安装”；索引、任务和版权登记仍可用，界面应禁用探测操作而不是伪造成功。能力不可用时请求返回固定错误，素材仍保持 `pending`，恢复组件后可以重试。
...
这两个方法才会返回 `absolute_path`，并且只返回 Electron 主进程。其响应绝不能经过 preload IPC 到 renderer；主进程最多将路径交给 `shell.showItemInFolder`。公开 DTO、运行日志和任务列表不得泄露原片或输出的绝对路径。
