...
function main(argv) {
  ...
  if (command === "send" && args.includes("--real")) return development.sendReal(baseDir, { allowRealSend: args.includes("--allow-real-send"), message: optionalValueAfter(args, "--message") });
  const handlers = {
