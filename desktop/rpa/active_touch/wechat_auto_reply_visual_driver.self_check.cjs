...
const screenFrameStart = AUTO_REPLY_VISUAL_SCRIPT.indexOf("function New-AutoReplyVisualScreenFrame");
const screenFrameEnd = AUTO_REPLY_VISUAL_SCRIPT.indexOf("function Get-AutoReplyVisualFrame", screenFrameStart);
assert.ok(screenFrameStart >= 0 && screenFrameEnd > screenFrameStart);
const screenFrameSource = AUTO_REPLY_VISUAL_SCRIPT.slice(screenFrameStart, screenFrameEnd);
assert.match(screenFrameSource, /GetForegroundWindow\(\) -ne \$hWnd[\s\S]*wechat_window_not_foreground/u, "screen-copy must abort when the exact preflight-bound HWND loses foreground");
assert.doesNotMatch(screenFrameSource, /ShowWindowAsync|SetForegroundWindow|AppActivate/u, "screen-copy must never reverse the user's focus change");
...
const openConversationStart = AUTO_REPLY_VISUAL_SCRIPT.indexOf("function Open-AutoReplyVisualConversation");
const openConversationEnd = AUTO_REPLY_VISUAL_SCRIPT.indexOf("function Get-AutoReplyVisualHeaderCandidates", openConversationStart);
assert.ok(openConversationStart >= 0 && openConversationEnd > openConversationStart);
const openConversationSource = AUTO_REPLY_VISUAL_SCRIPT.slice(openConversationStart, openConversationEnd);
assert.doesNotMatch(openConversationSource, /ShowWindowAsync|SetForegroundWindow|AppActivate/u, "an exact visual conversation transaction must fail closed instead of stealing foreground");
assert.match(openConversationSource, /GetForegroundWindow\(\) -ne \$hWnd[\s\S]*wechat_window_not_foreground/u);
assert.match(openConversationSource, /Test-AutoReplyVisualPointOwned[\s\S]*SetCursorPos[\s\S]*Test-AutoReplyVisualPointOwned[\s\S]*mouse_event/u, "every visual mouse input must be fenced by exact foreground and point ownership");
assert.match(AUTO_REPLY_VISUAL_SCRIPT, /\$hitRoot -eq \$expectedHWnd[\s\S]*\$hitPid -eq \$expectedPid[\s\S]*\$rootPid -eq \$expectedPid/u, "the fallback click must bind WindowFromPoint to the exact root HWND and PID");
assert.match(AUTO_REPLY_VISUAL_SCRIPT, /\$openedConversation = Open-AutoReplyVisualConversation[\s\S]*reason = \[string\]\$openedConversation\.reason/u, "focus loss must preserve its safe reason instead of becoming a generic open failure");
