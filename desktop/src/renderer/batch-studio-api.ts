...
export type Candidate = { candidate_id: string; title: string; narration: string; phrases?: { text: string }[]; angle: string; status: string; generated_video_id?: string; error?: string; duration_ms?: number; shots: Shot[]; actual_shots?: Shot[]; revision: number };
export type Batch = {
  ...
  settings: { voice_persona_id?: string; brand_profile_id?: string; minimum_duration_seconds?: number }; approved: boolean;
...
export const batchStatus: Record<string, string> = {
  draft: "待选材", planning: "正在准备作品", ready: "可以生成", insufficient_materials: "暂未得到合格作品",
