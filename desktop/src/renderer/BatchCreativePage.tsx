...
export function BatchCreativePage({ initial, onOpenProduct, onOpenLegacy, onOpenHistory, onOpenMaterials }: Props) {
  ...
  const [settings, setSettings] = useState<Batch["settings"]>(() => {
    try { return JSON.parse(localStorage.getItem("batch-studio-settings") || "{}"); } catch { return {}; }
  });
  ...
  return <div className="page batch-page">
    ...
    <div className="batch-toolbar"><label>当前批次<select aria-label="当前批次" value={batch?.batch_id || ""} disabled={busy} onChange={(e) => { if (e.target.value) void run(async () => load(await callBatch<Batch>("get", { batch_id: e.target.value }))); }}><option value="">新批次</option>{batches.map((b) => <option value={b.batch_id} key={b.batch_id}>{b.title} · {batchStatus[b.status] || b.status} · {b.completed_count || 0}/{b.target_count || "—"}</option>)}</select></label>
    ...
    {batch && <section className="batch-progress" aria-live="polite"><strong>{batchStatus[batch.status] || "处理中"}</strong><span>已完成 {completed} / {batch.target_count || "待定"} 条</span>{running && <span className="batch-processing">处理中，请稍候…</span>}
      {batch.task_id && (running || paused) && <><button disabled={busy} onClick={() => void run(async () => {
      ...
      })}>{paused ? batch.status === "outcome_unknown" ? "核对并尝试恢复" : "恢复任务" : "暂停"}</button><button disabled={busy} onClick={() => void run(async () => { const r = await window.xiaoxiContent?.tasks.cancel({ taskId: batch.task_id! }); if (!r?.ok) throw new Error(r?.error); setBatch(await callBatch<Batch>("get", { batch_id: batch.batch_id })); })}>取消本批任务</button></>}
    ...
    <details className="batch-source-details" open={completed === 0 || dirty}>
    ...
    <fieldset disabled={locked} className="batch-form">
      ...
      <p className="batch-hint">共 {total} 个素材。以上是初步安排，AI 会按内容调整位置；一个模块可以贡献多个镜头。</p>
      <div className="batch-count-bar"><label>本批生成数量<input aria-label="本批生成数量" type="number" min={1} max={300} step={1} value={count} placeholder="AI 分析后推荐" onChange={(e) => { manualCount.current = true; setCount(e.target.value); setDirty(true); }} /></label><button data-batch-action="recommend" disabled={!total} onClick={() => void start("recommend")}>AI 推荐数量</button><button data-batch-action="samples" className="batch-primary" disabled={!total} onClick={() => void start("samples")}>{batch?.status === "needs_attention" || batch?.status === "completed_with_errors" ? "继续制作未完成作品" : "生成样片"}</button></div>
      <p className="batch-hint">每批最多 300 条，实际数量需通过素材检查。先做最多 3 条样片；只要 1、2 条时直接完成。AI 分析与配音使用已配置的服务。</p>
      ...
      {batch?.reasons?.length ? <div className="batch-reasons" role="status">{batch.reasons.map((reason, i) => <p key={i}>{reason}</p>)}</div> : null}
    ...
    {batch && <details className="batch-advanced"><summary>项目详情</summary><p>流程：理解素材 → 规划与筛选 → 数量校验 → 样片 → 批量制作</p><p>任务状态：{batch.task_status || "尚未开始"}；已保存方案 {batch.candidates.length} 条。</p><p>数量来自有界搜索；未遍历全部空间时，不代表素材的绝对容量。</p></details>}
    ...
    {editing && batch && <CandidateEditor candidate={editing} batch={batch} onClose={() => setEditing(null)} onSaved={(b) => { setBatch(b); setEditing(null); setNotice("已保存本条修改。点击生成样片复核并制作修改后的作品，已完成的其他作品会保留。"); }} />}
  </div>;
}
...
export function BatchFinishedOverview({ onOpen }: { onOpen: (batchId: string) => void }) {
  ...
  return <section className="batch-finished-overview"><h2>按批次管理</h2>{notice && <p>{notice}</p>}<div className="batch-history-list">{batches.filter((b) => (b.completed_count || 0) > 0).map((b) => <article key={b.batch_id}><div><strong>{b.title}</strong><p>{b.completed_count} / {b.target_count} 条 · {batchStatus[b.status] || b.status}</p></div><button onClick={() => onOpen(b.batch_id)}>预览与导出批次</button></article>)}</div>{!batches.some((b) => (b.completed_count || 0) > 0) && <p>完成样片后，批次会显示在这里。</p>}</section>;
}
