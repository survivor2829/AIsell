...
type Props = {
  ...
  onOpen: (target: "contact-sync" | "expert" | "reply" | "diagnostics") => void;
...
export function WechatGettingStarted({ active, connected, aiConfigured, workflow, onOpen, onEditor, onClose }: Props) {
  ...
  const step = !connected ? "连接微信" : needsAiConnection ? "查看服务状态" : needsExpert && !expertReady ? "建立你的AI专家" : !saved ? choice === "reply" ? "选择回复联系人" : "填写任务并保存" : "确认后启动";
  const go = () => { if (!connected) onOpen("contact-sync"); else if (needsAiConnection) onOpen("diagnostics"); else if (needsExpert && !expertReady) onOpen("expert"); else if (choice === "reply") onOpen("reply"); else onEditor(taskType); };
  return <section className="wechat-getting-started" aria-label="微信拓客开始引导">
    ...
    {choice && <>
      ...
      <div className="wechat-guide-next"><div><strong>{step}</strong><p>{!connected ? "打开连接页面，确认当前微信并同步联系人。" : needsAiConnection ? "云端智能服务暂不可用，当前设置已保留；可稍后重试或在吐槽中心反馈。" : needsExpert && !expertReady ? "说明业务和接待方式，保存专家资料后回来继续。" : !saved ? choice === "reply" ? "在自动回复页面选定联系人并保存；只会回复你选中的人。" : "打开任务编辑器，选择对象、填写内容，并点击保存任务。" : "设置已保存。启动会执行当前计划中的任务，请先检查对象与内容。"}</p></div>{ready ? <WorkflowToggle workflow={workflow} /> : <button className="primary-button" onClick={go}>{step}<ChevronRight size={16} /></button>}</div>
