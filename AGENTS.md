...
- 故障先按设备、版本和执行阶段核对日志，区分已确认原因与待验证假设；保留具体原因及发送状态，不把不同错误统一成一个通用报错。
- 修复时沿故障步骤检查前后链路及共用适配器的调用方，修正有证据的同类假设，不只修入口，也不无依据扩大重写。验证覆盖实际触发条件（如最小化、空控件树、不同剪贴板内容），不只验证预先准备好的顺利场景；故障机复验结果按用户实际确认范围记录，再沉淀通用规则。
