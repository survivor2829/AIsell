...
测试版包含内部单联系人验收入口；阶段交付版隐藏开发入口。两个版本复用同一套真实发送事务和安全门禁，并包含白名单文字自动回复 MVP；朋友圈等其他产品入口仍标注为下一阶段开放。当前包不代表完整商品。
...
- `自动回复` 只处理白名单联系人的未读文字消息，支持工作时间、稳定去重、30 分钟联系人冷却、每日 20 条上限和 DeepSeek 回复；发送前会重新核对当前会话与最新来信，人工接管或内容变化时直接取消。
...
- 自动回复当前仅交付白名单文字 MVP；暂不处理群聊、图片、语音、文件、系统消息，也不接朋友圈点赞评论和短视频获客真实链路。
