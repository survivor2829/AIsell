...
> 最新变更：用户随后重新指定不带文字的机器人原图，去白边后已替换源码和桌面快捷方式图标；本次尚未重新打包，快捷方式目前引用项目ICO。见 [新主图标记录](2026-09-08-incremental-update-assessment.md)。以下1.0.2记录是之前一版图标的安装验收。

> 最终安装状态：1.0.2 测试版已安装，EXE 内七档图标与最终确认的带“AI获客”文字 ICO 一致。桌面仅保留测试版入口，图标指向安装后的 EXE，不再依赖项目目录；实际桌面截图已核对。旧正式版程序已卸载，数据及备份保留。详见 [1.0.2 验收](2026-09-08-internal-release-1.0.2.md)。下文为替换过程中的历史状态。
