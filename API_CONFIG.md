...
- 当前服务器组件已部署并通过健康检查；Provider 能力是否可用以 `/capabilities` 返回的布尔值为准。DeepSeek、APIMart 和火山引擎使用服务器受保护环境中的凭据，火山引擎的一把统一 Key 可覆盖 Ark、TTS、ASR。服务端尚未配置的能力会安全返回未配置，不会伪装成成功。
